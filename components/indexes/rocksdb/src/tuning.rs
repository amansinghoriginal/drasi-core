...
impl RocksDbTuning {
    ...
    fn apply_common(&self, opts: &mut Options, hot: bool) {
        ...
        // Retain about one flushed buffer generation for optimistic conflict
        // checks. Sessions are single-writer and short, so the 128 MiB
        // OptimisticTransactionDB sanitization default is never needed, and
        // without an explicit bound each flush would move memory into retained
        // history instead of freeing it.
        opts.set_max_write_buffer_size_to_maintain(wbs as i64);
