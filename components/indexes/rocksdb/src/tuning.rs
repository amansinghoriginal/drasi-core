...
impl RocksDbTuning {
    ...
    fn apply_common(&self, opts: &mut Options, hot: bool) {
        ...
        // Retain no flushed-memtable history. Pessimistic transactions take
        // row locks up front instead of validating against memtable history
        // at commit, so the OCC-era history window is unnecessary — and
        // retained memtables stay charged to the WriteBufferManager after
        // every flush, so any nonzero bound converts sustained ingest into
        // never-freed budget (measured under OCC: 2.8 GiB charged against a
        // 512 MiB budget with live memtables under 200 MiB).
    ...
    /// Base options for a CF: shared block cache, tiered write buffer,
    /// bounded flushed-memtable retention.
