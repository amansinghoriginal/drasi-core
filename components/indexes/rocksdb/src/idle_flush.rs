...
impl IdleFlushRegistry {
    ...
    /// One sweep pass. Returns the number of DBs flushed (for tests/logging).
            ...
            let cfs: Vec<_> = ALL_CFS
                ...
                .filter_map(|name| {
                    ...
                    let size = db
                        ...
                        .flatten()
                        .unwrap_or(0);
                    (size > FLUSH_THRESHOLD_BYTES).then_some(handle)
                })
                .collect();
            if !cfs.is_empty() {
                let mut opts = FlushOptions::default();
                opts.set_wait(false);
                let refs: Vec<&_> = cfs.iter().collect();
...
mod tests {
    ...
    async fn sweeps_idle_db_and_skips_active_ones() {
        ...
        assert_eq!(registry.sweep_once(), 0); // not idle (writes advanced seq)
        session.begin().unwrap();
        assert_eq!(registry.sweep_once(), 0); // idle but session active -> skipped
        session.rollback();
        assert_eq!(registry.sweep_once(), 1); // idle, no session -> flushed
    }
