...
pub(crate) struct SweepStats {
    ...
    /// Flushed-but-retained memtable bytes across all DBs (write buffer
    /// history) — charged to the WriteBufferManager but unfreeable by any
    /// flush; only bounded by max_write_buffer_size_to_maintain.
...
impl IdleFlushRegistry {
    ...
    pub(crate) fn sweep_once(&self) -> SweepStats {
        ...
        entries.retain_mut(|entry| {
            ...
            let sized_cfs: Vec<_> = ALL_CFS
                ...
                .filter_map(|name| {
                    ...
                    let size = db
                        ...
                        .flatten()
                        .unwrap_or(0);
                    // size-all additionally counts flushed-but-retained
                    // memtables (write buffer history); the difference is
                    // memory the WriteBufferManager still charges but no
                    // flush can ever free.
                    let size_all = db
                        .property_int_value_cf(&handle, "rocksdb.size-all-mem-tables")
                        .ok()
                        .flatten()
                        .unwrap_or(size);
                    Some((handle, size, size_all))
                })
                .collect();
            let total: u64 = sized_cfs.iter().map(|(_, size, _)| size).sum();
            stats.retained_bytes += sized_cfs
                .iter()
                .map(|(_, size, size_all)| size_all.saturating_sub(*size))
                .sum::<u64>();
            ...
            for (handle, size, _) in sized_cfs {
                    ...
                    if pressured_sweeps.is_multiple_of(BASELINE_SWEEP_TICKS) {
                        ...
                        log::info!(
                            "idle-flush ledger: pool {}/{} MiB; flushed {} DB(s) ({} MiB); \
                             ...
                             {} in-session DB(s) hold {} MiB; retained history {} MiB",
                            usage as u64 / MIB,
                            capacity as u64 / MIB,
                            stats.flushed,
