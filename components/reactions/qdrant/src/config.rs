...
pub struct EmbeddingConfig {
    /// Type of embedding service to use
    pub service_type: EmbeddingServiceType,

    /// Azure OpenAI endpoint (required for azure_openai service type)
    /// e.g., "https://myresource.openai.azure.com"
    #[serde(default)]
    pub endpoint: Option<String>,

    /// Model/deployment name (required for azure_openai service type)
    #[serde(default)]
    pub model: Option<String>,

    /// API key (required for azure_openai service type)
    #[serde(default)]
    pub api_key: Option<String>,
...
impl QdrantReactionConfig {
    ...
    pub fn validate(&self) -> anyhow::Result<()> {
        ...
        match self.embedding.service_type {
            EmbeddingServiceType::AzureOpenai => {
                if self.embedding.endpoint.is_none() {
                    anyhow::bail!(
                        "Azure OpenAI endpoint is required when service_type is azure_openai"
                    );
                }
                if self.embedding.model.is_none() {
                    anyhow::bail!(
                        "Azure OpenAI model is required when service_type is azure_openai"
                    );
                }
                if self.embedding.api_key.is_none() {
                    anyhow::bail!(
                        "Azure OpenAI API key is required when service_type is azure_openai"
                    );
                }
            }
            EmbeddingServiceType::Mock => {
                // No additional validation needed for mock embedder
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_missing_azure_openai_endpoint() {
        let config = QdrantReactionConfig {
            qdrant: QdrantConfig {
                endpoint: "http://localhost:6334".to_string(),
                api_key: None,
        ...
        assert!(err
            .to_string()
            .contains("Azure OpenAI endpoint is required"));
    }
    ...
    fn test_serde_roundtrip() {
        let config = QdrantReactionConfig {
            ...
            embedding: EmbeddingConfig {
                ...
                model: Some("embedding-model".to_string()),
        ...
        assert_eq!(
            config.embedding.service_type,
            deserialized.embedding.service_type
        );
        assert_eq!(
