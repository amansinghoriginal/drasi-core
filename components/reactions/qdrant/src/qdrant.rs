...
use crate::config::{EmbeddingServiceType, QdrantReactionConfig, RetryConfig};
use crate::document::{
...
///     EmbeddingConfig, EmbeddingServiceType, DocumentConfig, BatchConfig, RetryConfig, MockEmbedder,
/// };
/// use std::sync::Arc;
///
...
///             service_type: EmbeddingServiceType::Mock,
///             dimensions: 1536,
///             ..Default::default()
///         },
...
impl QdrantReaction {
    ...
    pub fn new(
        ...
        let embedder: Arc<dyn Embedder> = match config.embedding.service_type {
            EmbeddingServiceType::Mock => Arc::new(MockEmbedder::new(config.embedding.dimensions)),
            EmbeddingServiceType::AzureOpenai => {
                let endpoint = config
                    .embedding
                    .endpoint
                    .clone()
                    .context("Azure OpenAI endpoint required")?;
                let model = config
                    .embedding
                    .model
                    .clone()
                    .context("Azure OpenAI model required")?;
                let api_key = config
                    .embedding
                    .api_key
                    .clone()
                    .context("Azure OpenAI API key required")?;
                ...
                Arc::new(AzureOpenAIEmbedder::new(
                    ...
                    config.embedding.api_version.clone(),
                ))
            }
        };
...
impl QdrantReactionBuilder {
    ...
    pub fn build(self) -> Result<QdrantReaction> {
        ...
        let embedder: Arc<dyn Embedder> = if let Some(e) = self.embedder {
            ...
            match config.embedding.service_type {
                EmbeddingServiceType::Mock => {
            ...
            loop {
                ...
                if !upsert_data.is_empty() {
                    for chunk in upsert_data.chunks(config.batch.upsert_batch_size) {
                        ...
                        // Retry with exponential backoff - panic on persistent failure
                        ...
                        .unwrap_or_else(|e| {
                            panic!(
                                "[{reaction_id}] FATAL: Batch upsert failed after {} retries. \
                                 Cannot guarantee data consistency. {} items lost. Error: {e:#}",
                                config.retry.max_retries, chunk_len
                            );
                        });
                    }
                }
                ...
                if !delete_ids.is_empty() {
                    ...
                    .unwrap_or_else(|e| {
                        panic!(
                            "[{reaction_id}] FATAL: Batch delete failed after {} retries. \
                             Cannot guarantee data consistency. {} deletes lost. Error: {e:#}",
                            config.retry.max_retries, delete_count
                        );
                    });
...
mod tests {
    use super::*;
    use crate::config::{BatchConfig, DocumentConfig, EmbeddingConfig, QdrantConfig, RetryConfig};
