...
use drasi_reaction_qdrant::{
    generate_deterministic_uuid, BatchConfig, DocumentConfig, EmbeddingConfig,
    EmbeddingServiceType, MockEmbedder, QdrantConfig, QdrantReactionConfig, RetryConfig,
};
...
/// Test configuration validation failure for Azure OpenAI missing credentials
#[test]
fn test_config_validation_azure_missing_credentials() {
    let config = QdrantReactionConfig {
        qdrant: QdrantConfig {
            endpoint: "http://localhost:6334".to_string(),
            api_key: None,
    ...
    let result = config.validate();
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Azure OpenAI"));
}
