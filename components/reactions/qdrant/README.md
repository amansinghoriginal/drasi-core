...
use drasi_reaction_qdrant::{
    QdrantReaction, QdrantReactionConfig, QdrantConfig,
    EmbeddingConfig, EmbeddingServiceType, DocumentConfig, BatchConfig, RetryConfig, AzureOpenAIEmbedder,
};
use std::sync::Arc;
...
let config = QdrantReactionConfig {
    ...
    embedding: EmbeddingConfig {
        service_type: EmbeddingServiceType::AzureOpenai,
        dimensions: 3072,
...
| Name | Description | Type | Default |
|------|-------------|------|---------|
| `service_type` | Embedder type (`Mock` or `AzureOpenai`) | `EmbeddingServiceType` | **(Required)** |
| `dimensions` | Embedding vector dimensions | `usize` | **(Required)** |
| `endpoint` | Azure OpenAI endpoint | `Option<String>` | `None` |
| `model` | Model deployment name | `Option<String>` | `None` |
| `api_key` | Azure OpenAI API key | `Option<String>` | `None` |
| `api_version` | Azure OpenAI API version | `Option<String>` | `"2024-02-01"` |
...
```rust
use drasi_reaction_qdrant::{QdrantReaction, QdrantReactionConfig, QdrantConfig, EmbeddingConfig, EmbeddingServiceType, DocumentConfig, BatchConfig, MockEmbedder};
use std::sync::Arc;
...
```rust
use drasi_reaction_qdrant::{QdrantReaction, QdrantReactionConfig, QdrantConfig, EmbeddingConfig, EmbeddingServiceType, DocumentConfig, BatchConfig, AzureOpenAIEmbedder};
use std::sync::Arc;
