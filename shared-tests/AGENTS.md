...
*   **Session-Aware Index Tests**: Index-level tests (`src/index`) accept an `Arc<dyn SessionControl>` and wrap all writes in `begin()`/`commit()` sessions. Callers pass `NoOpSessionControl` for in-memory or the backend's real `SessionControl` for persistent backends.
